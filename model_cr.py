...
class DecoderAlpha(nn.Module):
    def __init__(
        ...
        self.conv1 = StyledConv(in_channel, in_channel, 3, style_dim, blur_kernel=blur_kernel)
        self.to_rgb1 = ToRGBA(in_channel, style_dim, upsample=False)
        ...
        for i in range(in_log_size+1, self.log_size + 1):
            ...
            self.to_rgbs.append(ToRGBA(out_channel, style_dim))
    ...
    def forward(self, input, styles):
        ...
        alpha = torch.clamp(0.1*skip[:,3:4,:,:]+0.5, 0.0, 1.0)
        # print(alpha)
        # print(image)
        return image, alpha
...
class Generator2(nn.Module):
    def __init__(
        ...
        self.decoder = Decoder(size, num_down, latent_dim, n_mlp, n_res)
    ...
    def decode(self, input, styles, use_mapping=True):
        image = self.decoder(input, styles)
        #print(image)
        return image
...
class Generator2_alpha(nn.Module):
    def __init__(
        self,
        size,
        num_down,
        ...
        self.encoder = Encoder(size, latent_dim, num_down, n_res, channel_multiplier)
        # self.encoder = MobileNetV3_Mogai()
        self.decoder = DecoderAlpha(size, num_down, latent_dim, n_mlp, n_res)
    ...
    def forward(self, input, z=None):
        content, style = self.encode(input)
        if z is None:
            out, alpha = self.decode(content, style)
        else:
            out, alpha = self.decode(content, z)
...
class Generator(nn.Module):
    def __init__(
        ...
        #self.fencoder = nn.Conv2d(3, 256, kernel_size=3, stride=1, padding=1, bias=False)
