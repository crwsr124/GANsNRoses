...
def test(args, genA2B, T_Encoder, T_Decoder, testA_loader, testB_loader, name, step):
    ...
    with torch.no_grad():
        ...
        for i in range(test_sample_num):
            ...
            fake_A2B_1, alpha1, img5121 = genA2B.decode(A2B_content, A2B_mod1)
            alpha1 = alpha1.repeat(1, 3, 1, 1)
            img5121 = F.interpolate(img5121, size=[256, 256], mode="bilinear")

            fake_A2B_2, alpha2, img5122 = genA2B.decode(A2B_content, A2B_mod2)
            alpha2 = alpha2.repeat(1, 3, 1, 1)
            img5122 = F.interpolate(img5122, size=[256, 256], mode="bilinear")

            colsA += [fake_A2B_1, alpha1, img5121, fake_A2B_2, alpha2, img5122]
...
def train(args, trainA_loader, trainB_loader, testA_loader, testB_loader, G_A2B, D_B, G_optim, D_optim, device, T_Decoder, T_Encoder):
    ...
    for idx in pbar:
        ...
        fake_A2B_t_512 = F.interpolate(fake_A2B_t, size=[512, 512], mode="bilinear")
        decoder_l512 = (10*F.l1_loss(image512, fake_A2B_t_512))
        ...
        G_loss = 100*decoder_l11 + 100*decoder_lpips1  + decoder_lalpha + decoder_l512 +\
...
if __name__ == '__main__':
    ...
    D_B = Discriminator512(512).to(device)
    ...
    augB = nn.Sequential(
        K.RandomAffine(degrees=(-20,20), scale=(0.8, 1.2), translate=(0.1, 0.1), shear=0.15),
        kornia.geometry.transform.Resize(512+30),
        K.RandomCrop((512,512)),
        K.RandomHorizontalFlip(),
    )
